...
//  private final List<Object> resultHolder = new ArrayList<>();
  ...
  public Mono<?> handleResultSet(Result result) {
    ...
    if (result != null && resultMap != null) {
      return handleResult(result, resultMap, null).collectList()
        .flatMap(list -> {
          String[] resultSets = mappedStatement.getResultSets();
          ResultMapping parentMapping = nextResultMaps.get(resultSets[handledResultSetCount.get()]);
          if (parentMapping != null) {
            String nestedResultMapId = parentMapping.getNestedResultMapId();
            ResultMap nestedResultMap = configuration.getResultMap(nestedResultMapId);
            return handleResult(result, nestedResultMap, parentMapping).collectList()
              .then(Mono.just(list));
          }
  ...
  private Flux<?> handleRowValuesForSimpleResultMap(Result result, ResultMap resultMap, ResultHandler<?> resultHandler, ResultMapping parentMapping) {
    return Flux.from(result.map((row, rowMetadata) -> {
      ...
      try {
        Object rowValue = handleRowValue(rowResultWrapper, resultMap, resultHandler, parentMapping);
        storeObject(resultHandler, resultContext, rowValue, parentMapping, rowResultWrapper);
  ...
  private Flux<?> handleRowValuesForNestedResultMap(Result result, ResultMap resultMap, ResultHandler<?> resultHandler, ResultMapping parentMapping) {
    return Flux.from(result.map((row, rowMetadata) -> {
      RowResultWrapper rowResultWrapper = new RowResultWrapper(row, rowMetadata, configuration);
      return rowResultWrapper;
    })).concatMap(rowResultWrapper -> {
